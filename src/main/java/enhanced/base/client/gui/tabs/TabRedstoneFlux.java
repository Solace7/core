...
public class TabRedstoneFlux extends BaseTab {
    ...
    public TabRedstoneFlux(BaseGui gui, IEnergyHandler h) {
        super(gui);
        name = Localisation.get(ECMod.ID, "tab.redstoneFlux");
        handler = h;
        maxHeight = 110;
    ...
    public void drawFullyOpened() {
        parent.getFontRenderer().drawStringWithShadow(Localisation.get(ECMod.ID, "tab.redstoneFlux.maxPower"), posX + 10, posY + 20, 0xAAAAAA);
        parent.getFontRenderer().drawString(handler.getMaxEnergyStored(ForgeDirection.UNKNOWN) + " RF", posX + 17, posY + 32, 0x000000);

        parent.getFontRenderer().drawStringWithShadow(Localisation.get(ECMod.ID, "tab.redstoneFlux.storedPower"), posX + 10, posY + 45, 0xAAAAAA);
        parent.getFontRenderer().drawString(handler.getEnergyStored(ForgeDirection.UNKNOWN) + " RF", posX + 17, posY + 57, 0x000000);

        parent.getFontRenderer().drawStringWithShadow(Localisation.get(ECMod.ID, "tab.redstoneFlux.powerUsage"), posX + 10, posY + 70, 0xAAAAAA);
